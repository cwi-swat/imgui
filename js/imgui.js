...
Result:
    ...
    for (ul("todos")) {
        for (var idx in todo.items) {
            for (li()) {
                label("lab" + idx, todo.items[idx].label); 
                if (let ev <- checkBox("chk" + idx, todo.items[idx].done)) {
                    todo.items[idx].done = GUI.checked;
                }
                if (button("del" + idx, "Delete")) 
        ...
        for (li()) {
            ...
            if (let newTodo <- textbox("new", newItemLabel)) 
