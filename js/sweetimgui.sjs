...
macro ul { 
    rule { { $body... } } => { tag("ul", [], function() { $body... }) }
    rule { ($x...) { $body... } } => { tag("ul", [$x...], function() { $body... }) }
}
macro li { 
    rule { { $body... } } => { tag("li", [], function() { $body... }) }
    rule { ($x...) { $body... } } => { tag("li", [$x...], function() { $body... }) }
}
...
function tag(name, cls, block) {
    var elt = $("<" + name + "></" + name + ">"); //document.createElement(name);
    if (cls.length > 0) {
        elt.addClass(cls[0]);
    }
    var parent = FOCUS;
